...
void *convoluteThreads(void *args)
{
    ...
    enum KernelTypes algorithm = threadArgs->algorithm;
    long rank = threadArgs->rank;

    int rowsPerThread = srcImage->height / thread_count;

    int row, pix, bit, span, start, end;

    start = rank * rowsPerThread;

    if(((srcImage->height/thread_count) * (rank+1)) < srcImage->height){
        end = (srcImage->height /thread_count) * (rank+1);
    }else{
        end = srcImage->height;
    }

    /*     int local_height = srcImage->height / thread_count;
        start = (rank * local_height);
        end = (rank + 1) * local_height - 1; */
    span = srcImage->bpp * srcImage->bpp;
    ...
    for (i = start; i < end; i++)
    {
        for (pix = 0; pix < srcImage->width; pix++)
        {
            for (bit = 0; bit < srcImage->bpp; bit++)
            {
                destImage->data[Index(pix, i, srcImage->width, bit, srcImage->bpp)] = getPixelValue(srcImage, pix, i, bit, algorithms[algorithm]);
            }
        }
    }
}
